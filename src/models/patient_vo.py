"""Patient value object for UI and Chroma DB metadata interchange.

This module defines PatientVO, a lightweight, JSON-serializable value object
suited for passing patient data between UI layers and Chroma vector store
...
@dataclass
class PatientVO:
    """Value object for patient data used by UI and Chroma DB metadata.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Return a JSON-serializable dict representation of the PatientVO."""
        return asdict(self)

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "PatientVO":
        """Create a PatientVO from a mapping. Accepts date strings or date objects.
    ...
    def with_updates(self, **kwargs: Any) -> "PatientVO":
        ...
        data = self.to_dict()
        data.update(kwargs)
        return PatientVO.from_dict(data)

    def __repr__(self) -> str:  # pragma: no cover - readability
        return (
            f"PatientVO(patient_id={self.patient_id!r}, name={self.full_name!r}, "
